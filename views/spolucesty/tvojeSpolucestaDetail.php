...
<div class="text-black dark:text-white dark:bg-darkBackground pb-10">
    <div class="px-4 py-8">
        <h1 class="text-3xl font-bold text-center mb-8">Uprav spolucestu</h1>
        <div>
            <form method="POST" action="<?= getUrl("spolucesta/updatetour/".$tour['tour_id'] ) ?>" >
            ...
            <input type="hidden" name="id" value="<?= $tour['tour_id'] ?>" />
                ...
                <div class="mb-4">
                    ...
                    <div class="relative w-full px-2">
                        <input
                            ...
                            value="<?= htmlspecialchars($tour['tour_destination']) ?>"
                        />
                ...
                <div class="flex gap-4 items-center mt-4">
                    ...
                    <input
                        ...
                        placeholder="MM/YYYY"
                        value="<?= htmlspecialchars(date('Y/m/d', strtotime($tour['tour_tourdate']))) ?>"
                    />
                    ...
                    <input
                        ...
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        placeholder="MM/YYYY"
                        value="<?= htmlspecialchars(date('Y/m/d', strtotime($tour['tour_tourdateEnd']))) ?>"
                    />
                </div>
                ...
                <div class="mb-4 text-sm mt-4">
                    ...
                    <?php foreach ($typeOfTourObjects as $index => $type): ?>
                        ...
                        <div class="mb-2 flex items-center w-[180px]">
                            <input class="mr-2 hidden" 
                                   id="journey-type-<?= $index ?>" 
                                   type="checkbox" 
                                   value="<?= htmlspecialchars($type->value) ?>" 
                                   name="journey_type[]"
                                   <?php if (in_array($type->value, $tourTypesArray)): ?>
                                        checked
                                    <?php endif; ?>
                                   >
                            <label for="journey-type-<?= $index ?>" class="relative flex cursor-pointer">
                                <div class="w-6 h-6 border border-gray-300 rounded-md flex items-center justify-center bg-white mr-2">
                                    <!-- Checkbox SVG icon -->
                                    <svg class="w-4 h-4 text-red-900 hidden" 
                                         fill="none" 
                                         viewBox="0 0 24 24" 
                                         stroke="currentColor">
                                        <path 
                ...
                <div class="mb-4">
                    ...
                    <textarea
                        ...
                        maxlength="500"
                        style="resize: none;"
                    ><?= htmlspecialchars($tour['tour_fellowtraveler']) ?></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea
                        ...
                        maxlength="500"
                        style="resize: none;"
                    ><?= htmlspecialchars($tour['tour_aboutme']) ?></textarea>
                </div>
                ...
                <a
                    ...
                    class=" bg-gray-500 hover:bg-gray-700  text-white font-bold py-2 px-8 rounded focus:outline-none focus:shadow-outline "
                    href="<?= getUrl('tvojespolucesty') ?>" 
                >
                    Zpět
                        </a>
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
                ...
                // Ensure the SVG icon visibility is correct on page load
                if (checkbox.checked) {
